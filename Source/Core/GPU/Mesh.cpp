...
{
    glm::vec3 minCoords = positions[0];
    glm::vec3 maxCoords = positions[0];
    for (glm::vec3 v : positions) {
        minCoords.x = minCoords.x < v.x ? minCoords.x : v.x;
        minCoords.y = minCoords.y < v.y ? minCoords.y : v.y;
        minCoords.z = minCoords.z < v.z ? minCoords.z : v.z;

        maxCoords.x = maxCoords.x > v.x ? maxCoords.x : v.x;
        maxCoords.y = maxCoords.y > v.y ? maxCoords.y : v.y;
        maxCoords.z = maxCoords.z > v.z ? maxCoords.z : v.z;
    }
