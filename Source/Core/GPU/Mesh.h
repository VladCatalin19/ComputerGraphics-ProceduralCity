...
struct AABB {
    ...
    AABB(glm::vec3 minCoords = glm::vec3(0.0f), glm::vec3 maxCoords = glm::vec3(0.0f))
    ...
    bool intersects(const AABB& box) {
        return (minCoords.x <= box.minCoords.x && maxCoords.x >= box.minCoords.x) &&
               (minCoords.y <= box.minCoords.y && maxCoords.y >= box.minCoords.y) &&
               (minCoords.z <= box.minCoords.z && maxCoords.z >= box.minCoords.z);
    }
};
